...
class CLAModel(nn.Module):
    """消融实验：去除 BiLSTM，仅保留 CNN + Attention"""
    ...
    def __init__(self):
        ...
        self.ncp_attn = nn.MultiheadAttention(128, 8, batch_first=True)
        ...
        self.oh_attn = nn.MultiheadAttention(128, 8, batch_first=True)

        self.adaptive_pool = nn.AdaptiveMaxPool1d(1)
        self.classifier = nn.Sequential(
            nn.Linear(128 + 128, 256), nn.ReLU(), nn.Dropout(0.2),
            nn.Linear(256, 64), nn.ReLU(), nn.Linear(64, 1), nn.Sigmoid()
        )

    def forward(self, ncp, dpcp):
        # NCP 分支：CNN → Attention
        ...
        na_t = na.permute(0, 2, 1)
        na_at, _ = self.ncp_attn(na_t, na_t, na_t)
        na_v = self.adaptive_pool(na_at.permute(0, 2, 1)).squeeze(-1)

        # DPCP 分支：CNN → Attention
        ...
        ob_t = ob.permute(0, 2, 1)
        ob_at, _ = self.oh_attn(ob_t, ob_t, ob_t)
